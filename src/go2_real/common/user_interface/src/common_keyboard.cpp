...
private:
    ...
    ros::Publisher topology_node_set_pub_;
    ...
    angular_z(0)
{
    ...
    topology_node_set_pub_ = nh_.advertise<std_msgs::Bool>("/topology_node_set", 1);
    running_ = true;
...
void KeyboardControl::keyLoop()
{
    ...
    while (ros::ok())
    {
        ...
        switch(c)
        {
            ...
            case KEYCODE_K:
                {
                    // 发布Bool消息到/topology_node_set话题
                    std_msgs::Bool msg;
                    msg.data = true;
                    topology_node_set_pub_.publish(msg);
                    ROS_INFO("发布拓扑节点保存命令");
                }
                break;
        }
