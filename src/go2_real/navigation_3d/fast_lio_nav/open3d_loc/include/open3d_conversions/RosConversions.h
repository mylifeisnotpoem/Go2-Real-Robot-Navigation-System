...
/*!
 ...
 can't use float literals as template parameters
    ...
    /*!
     * \brief Convert Eigen::Matrix4d into geometry_msgs::Pose.
     * \param  m is Eigen 4x4 matrix to be converted into a geometry_msgs::Pose.
     * \return tf::Pose 
     */
    template<>
    inline geometry_msgs::Pose Convert<Eigen::Matrix4d, geometry_msgs::Pose>(Eigen::Matrix4d m) {
        tf::Pose pose;
        Eigen::Vector3d trans(m.block<3, 1>(0, 3));
        Eigen::Quaterniond q(m.block<3, 3>(0, 0));
        pose.setRotation(tf::Quaternion(q.x(), q.y(), q.z(), q.w())); // pose.getRotation());
    ...
    /*!
     * \brief Convert tf::Pose into Eigen::Matrix4d.
     * \param  m is Eigen 4x4 matrix to be converted into a tf::Pose.
     * \return tf::Pose 
     */
    template<>
    inline Eigen::Matrix4d Convert<tf::Pose, Eigen::Matrix4d>(tf::Pose m) {
    ...
    template<>
    inline Eigen::Matrix4d Convert<geometry_msgs::Pose, Eigen::Matrix4d>(geometry_msgs::Pose m) {
        tf::Pose pose = tf::Pose(tf::Quaternion(m.orientation.x, m.orientation.y, m.orientation.z, m.orientation.w),
                tf::Vector3(m.position.x, m.position.y, m.position.z));
    ...
    template<>
    inline Eigen::Isometry3d Convert<geometry_msgs::Pose, Eigen::Isometry3d>(geometry_msgs::Pose m) {
        tf::Pose pose = tf::Pose(tf::Quaternion(m.orientation.x, m.orientation.y, m.orientation.z, m.orientation.w),
                tf::Vector3(m.position.x, m.position.y, m.position.z));
        Eigen::Matrix4d mid;
        Eigen::Translation3d tl_btol(
        ...
        rotation_matrix << mid(0, 0), mid(0, 1), mid(0, 2),
        mid(1, 0), mid(1, 1), mid(1, 2),
        mid(2, 0), mid(2, 1), mid(2, 2);
        result.rotate(rotation_matrix);
        Eigen::Vector3d t1;
        t1 << mid(0, 3), mid(1, 3), mid(2, 3);
    ...
    template<>
    inline Eigen::Vector3d Convert<geometry_msgs::Quaternion, Eigen::Vector3d> (geometry_msgs::Quaternion v) {
        double r_w = v.w;
		double r_x = v.x;
		double r_y = v.y;
		double r_z = v.z;
