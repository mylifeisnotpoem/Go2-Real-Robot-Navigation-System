                                                                                             ...
                                                                                             nh_private_(nh_private)
{
    ...
    GetTfTransformToMatrix("base_link", "laser_link", mat_imulink2baselink_);
    ...
    while (1)
    {
        ...
        else
        {
            ...
            *target = *map_fine_crop;
            ...
            // 最小影响防护：子图为空或点云异常时跳过本次定位，避免节点崩溃退出
            if (target->IsEmpty())
            {
                lock_mat_odom2map_.unlock();
                ROS_WARN_THROTTLE(1.0, "Localization skip: target(submap) is empty, keep last odom2map");
                continue;
            }
            ...
            if (!target->HasNormals())
            {
                target->EstimateNormals(
                    open3d::geometry::KDTreeSearchParamHybrid(std::max(0.05, voxelsize_fine_ * 2.0), 30));
            }

            if (source->IsEmpty())
            {
                lock_mat_odom2map_.unlock();
                ROS_WARN_THROTTLE(1.0, "Localization skip: source(scan) is empty after crop/downsample");
                continue;
            }
            ...
            {
                lock_mat_odom2map_.unlock();
                ROS_ERROR_THROTTLE(1.0, "Localization skip: ICP exception: %s", e.what());
                continue;
            }
            /// 给发布的置信度赋值
