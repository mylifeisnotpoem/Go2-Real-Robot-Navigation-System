...
>>>>>>> cec3d0ec371a81dd56b606f290c5fd43da59fbc4
    ...
    nav_msgs::Path GlobalPathPlanner::planGlobalPath()
    {
        ...
        try
        {
            ...
            // === 修改：根据起终点直线距离决定使用哪种算法 ===
            std::vector<int> topology_path;
            if (direct_distance > astar_distance_threshold_)
            {
                ROS_INFO("起终点距离 %.2f 米 > 阈值 %.2f 米，启用 A* 算法", direct_distance, astar_distance_threshold_);
                topology_path = aStarSearchInTempGraph(temp_nodes, temp_edges, start_node_id, goal_node_id);
            }
            else
            {
                ROS_INFO("起终点距离 %.2f 米 <= 阈值 %.2f 米，启用 Dijkstra 算法", direct_distance, astar_distance_threshold_);
                topology_path = dijkstraSearchInTempGraph(temp_nodes, temp_edges, start_node_id, goal_node_id);
            }
    ...
    std::vector<int> GlobalPathPlanner::dijkstraSearch(int start_id, int goal_id)
    {
        ...
        //A*算法
    std::vector<int> GlobalPathPlanner::aStarSearchInTempGraph(const std::vector<TopologyNode> &temp_nodes,
                                                               const std::vector<TopologyEdge> &temp_edges,
                                                               int start_id, int goal_id)
    {
        // 建立节点映射，方便快速获取节点坐标来计算 H 值
        ...
        if (temp_node_map.find(goal_id) == temp_node_map.end()) {
            return std::vector<int>(); // 防御性检查
        }
        TopologyNode goal_node = temp_node_map[goal_id];

        // 对应开源代码的 G 值 (起点到当前点的已知最短代價)
        std::map<int, double> g_scores;
        // 对应开源代码的 F 值 (F = G + H)
        std::map<int, double> f_scores; 
        // 对应开源代码的 node->parent
        std::map<int, int> previous;

        // 优先队列 OpenList，按照 F 值从小到大排序
        std::priority_queue<std::pair<double, int>,
                            std::vector<std::pair<double, int>>,
        ...
        for (const auto &node : temp_nodes)
        {
            g_scores[node.id] = std::numeric_limits<double>::infinity();
            f_scores[node.id] = std::numeric_limits<double>::infinity();
        }

        // 起点初始化
        g_scores[start_id] = 0.0;
        f_scores[start_id] = calculateHeuristic(temp_node_map[start_id], goal_node);
        pq.push(std::make_pair(f_scores[start_id], start_id));
        ...
        while (!pq.empty())
        {
            ...
            // 过滤旧数据 (如果弹出的 F 值比记录的还要大，说明是过期节点)
            if (current_f > f_scores[current_id])
            {
                continue;
            }
            ...
            for (const auto &edge : temp_edges)
            {
                ...
                if (neighbor_id != -1)
                {
                    // 计算从起点经过 current_id 到 neighbor_id 的新 G 值
                    ...
                    if (tentative_g_score < g_scores[neighbor_id])
                    {
                        ...
                        // 计算 H 值和 F 值 (F = G + H)
                        double h_score = calculateHeuristic(temp_node_map[neighbor_id], goal_node);
                        f_scores[neighbor_id] = g_scores[neighbor_id] + h_score;
                        
                        // 将邻居加入 OpenList
                        pq.push(std::make_pair(f_scores[neighbor_id], neighbor_id));
                    }
                }
            }
        }

        // --- 路径重构 (与 Dijkstra 逻辑完全一致) ---
        std::vector<int> path;
        if (g_scores[goal_id] == std::numeric_limits<double>::infinity())
        {
            ROS_WARN("A* 在临时拓扑图中未找到从 %d 到 %d 的路径", start_id, goal_id);
            return path;
        }
        ...
        while (current != start_id)
        {
            ...
            {
                ROS_ERROR("A* 路径重构失败：节点 %d 没有前驱节点", current);
        ...
        ROS_INFO("A* 算法在临时拓扑图中找到路径，包含 %lu 个节点", path.size());
        return path;
    }

        //A*启发式函数
    double GlobalPathPlanner::calculateHeuristic(const TopologyNode& node, const TopologyNode& goal)
    {
        // 对应开源代码里的 Euclidean H值计算
